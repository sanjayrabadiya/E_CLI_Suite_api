...
namespace GSC.Report
{
    public class ReportSyncfusion : IReportSyncfusion
    {
        ...
        public ReportSyncfusion(Microsoft.AspNetCore.Hosting.IWebHostEnvironment hostingEnvironment,
        ...
        public async Task<string> DossierPdfReportGenerate(ReportSettingNew reportSetting, JobMonitoring jobMonitoring)
        {
            ...
            var docPath = await _uploadSettingRepository.All.OrderByDescending(x => x.Id).FirstOrDefaultAsync();
            if (docPath != null)
                base_URL = docPath.DocumentPath;

            var parent = _context.Project.Where(x => x.Id == reportSetting.ProjectId).FirstOrDefault();
            if (parent != null)
        ...
        private void DesignTemplateReport(IList<ProjectDesignTemplatelist> designtemplate, ReportSettingNew reportSetting, string vistitName, PdfPage sectioncontent, int ProjectDesignId)
        {
            ...
            foreach (var designt in designtemplate.OrderBy(i => i.DesignOrder))
            {
                ...
                {
                    if (reportSetting.AnnotationType)
                    {
                ...
                for (int n = 0; n < designt.TemplateNotes.Count; n++)
                {
                    if (designt.TemplateNotes[n].IsPreview)
                        notes.Append(designt.TemplateNotes[n].Notes + "\n ");
                }
                if (!string.IsNullOrEmpty(notes.ToString()))
                    result = AddString($"{notes.ToString()}", result.Page, new Syncfusion.Drawing.RectangleF(0, result.Bounds.Bottom, 400, result.Page.GetClientSize().Height), PdfBrushes.Black, italicfont, layoutFormat);
                        ...
                        foreach (var variable in variableList)
                        {
                            ...
                            else if (variable.CollectionSource == CollectionSources.CheckBox)
                            {
                                ...
                                else
                                {
                                    foreach (var value in variable.Values)
                                    {
                                        ...
                                        checkField.Bounds = new RectangleF(350, result.Bounds.Y, 15, 15);
                                        checkField.Style = PdfCheckBoxStyle.Check;
                                        if (!String.IsNullOrEmpty(variable.ScreeningValue) && variable.ScreeningValue == "true")
                                        {
                                            checkField.Checked = true;
                            ...
                            else if (variable.CollectionSource == CollectionSources.Date)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(variable.ScreeningValue))
                                    {
                                        if (DateTime.TryParse(variable.ScreeningValue, CultureInfo.InvariantCulture, out dDate))
                                        {
                                            dt = DateTime.Parse(variable.ScreeningValue, CultureInfo.InvariantCulture).UtcDateTime().ToString(GeneralSettings.DateFormat);
                                        }
                                    }
                                    else
                                    {
                                        dt = variable.ScreeningValue;
                                    }
                            ...
                            else if (variable.CollectionSource == CollectionSources.DateTime)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(variable.ScreeningValue))
                                    {
                                        if (DateTime.TryParse(variable.ScreeningValue, CultureInfo.InvariantCulture, out dDate))
                                        {
                                            dttime = DateTime.Parse(variable.ScreeningValue, CultureInfo.InvariantCulture).UtcDateTime().ToString(GeneralSettings.DateFormat + ' ' + GeneralSettings.TimeFormat);
                                        }
                                    }
                                    else
                                    {
                                        dttime = variable.ScreeningValue;
                                    }
                                    ...
                                    if (variable.Values != null)
                                    {
                                        ...
                                        variable.Values.ToList().ForEach(val =>
                                        {
                                            ...
                                            .Select(x => new ScreeningTemplateValueChild
                                            {
                                                ...
                                                ScreeningTemplateValueId = x.Select(s => s.ScreeningTemplateValueId).FirstOrDefault(),
                                                ProjectDesignVariableValueId = x.Select(s => s.ProjectDesignVariableValueId).FirstOrDefault(),
                                            ...
                                            childValue.ForEach(child =>
                                            {
                                                ...
                                                obj.IsDeleted = child.DeletedDate != null;
                                                obj.TableCollectionSource = val.TableCollectionSource;
                                                ValuesList.Add(obj);
                                            });
                                        });

                                        var Values = ValuesList.Where(x => !x.IsDeleted).ToList();
                                        ...
                                        if (Values != null && Values.Count > 0)
                                        {
                                            ...
                                            foreach (var row in rowdata)
                                            {
                                                ...
                                                foreach (var finalrow in row1)
                                                {
                                                    ...
                                                    {
                                                        if (!string.IsNullOrEmpty(finalrow.ScreeningValue))
                                                        {
                                                            if (DateTime.TryParse(finalrow.ScreeningValue, CultureInfo.InvariantCulture, out dDate))
                                                            {
                                                                value = DateTime.Parse(finalrow.ScreeningValue, CultureInfo.InvariantCulture).UtcDateTime().ToString(GeneralSettings.DateFormat + ' ' + "hh:mm tt");
                                                            }
                                                            else
                                                    ...
                                                    else if (finalrow.TableCollectionSource == TableCollectionSource.Date)
                                                    {
                                                        if (!string.IsNullOrEmpty(finalrow.ScreeningValue))
                                                        {
                                                            if (DateTime.TryParse(finalrow.ScreeningValue, CultureInfo.InvariantCulture, out dDate))
                                                            {
                                                                value = DateTime.Parse(finalrow.ScreeningValue, CultureInfo.InvariantCulture).UtcDateTime().ToString(GeneralSettings.DateFormat);
                                                            }
                                                            else
                                                                value = finalrow.ScreeningValue;
                                                        }
                                                    ...
                                                    else if (finalrow.TableCollectionSource == TableCollectionSource.Time)
                                                    {
                                                        if (!string.IsNullOrEmpty(finalrow.ScreeningValue))
                                                        {
                                                            if (DateTime.TryParse(finalrow.ScreeningValue, CultureInfo.InvariantCulture, out dDate))
                                                            {
                                                                value = DateTime.Parse(finalrow.ScreeningValue, CultureInfo.InvariantCulture).UtcDateTime().ToString("hh:mm tt");
                                                            }
                                                            else
                                                                value = finalrow.ScreeningValue;
                                                        }
                            ...
                            if (secondresult.Page == thirdresult.Page && secondresult.Bounds.Bottom > thirdresult.Bounds.Bottom && thirdresult.Bounds.Height < secondresult.Bounds.Height)
                        ...
                        foreach (var variable in variableList)
                        {
                            ...
                            else if (variable.CollectionSource == CollectionSources.Date)
                            {
                                ...
                                else
                                {
                                    ...
                                    string dt = string.Empty;
                                    if (!string.IsNullOrEmpty(variable.ScreeningValue) && DateTime.TryParse(variable.ScreeningValue, CultureInfo.InvariantCulture, out dDate))
                                    {
                                        dt = DateTime.Parse(variable.ScreeningValue, CultureInfo.InvariantCulture).UtcDateTime().ToString(GeneralSettings.DateFormat);

                                    }
                                    else if (!string.IsNullOrEmpty(variable.ScreeningValue))
                                    {
                                        dt = variable.ScreeningValue;
                                    }
                ...
                for (int n = 0; n < designt.TemplateNotesBottom.Count; n++)
                {
                    if (designt.TemplateNotesBottom[n].IsPreview)
                        notesb.Append(designt.TemplateNotesBottom[n].Notes + "\n ");
                }
                if (!string.IsNullOrEmpty(notesb.ToString()))
                    result = AddString($"{notesb.ToString()}", result.Page, new Syncfusion.Drawing.RectangleF(0, result.Bounds.Bottom, 400, result.Page.GetClientSize().Height), PdfBrushes.Black, italicfont, layoutFormat);
                ...
                foreach (var workflow in workflowlevel)
                {
                    ...
                    {
                        if (level.IsElectricSignature && designt.ScreeningTemplateReview != null)
                        {
                            var signature = designt.ScreeningTemplateReview.LastOrDefault(s => s.ReviewLevel > level.LevelNo - 1 && s.RoleId == level.SecurityRoleId);
                            if (signature != null)
                            {
        ...
        public async Task<string> ScreeningPdfReportGenerate(ScreeningReportSetting reportSetting, JobMonitoring jobMonitoring)
        {
            ...
            string base_URL = string.Empty;
            var upload = await _uploadSettingRepository.All.OrderByDescending(x => x.Id).FirstOrDefaultAsync();
            if (upload != null)
                base_URL = upload.DocumentPath;

            var parent = await _context.Project.Where(x => x.Id == reportSetting.ProjectId).FirstOrDefaultAsync();
            if (parent != null)
                fileInfo.ParentFolderName = parent.ProjectCode + "_" + DateTime.Now.Ticks;
