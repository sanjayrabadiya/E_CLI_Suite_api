...
{
    public class ProjectScheduleRepository : GenericRespository<ProjectSchedule, GscContext>, IProjectScheduleRepository
    {
        public ProjectScheduleRepository(IUnitOfWork<GscContext> uow, IJwtTokenAccesser jwtTokenAccesser) : base(uow,
            jwtTokenAccesser)
        {
        }
        ...
        {
            var projectScheduleTemplate = (from projectschedule in Context.ProjectSchedule
                    .Where(t => t.DeletedBy == null).Where(x =>
                        x.ProjectDesignId == projectId && x.ProjectDesignPeriodId == periodId)
                join projectscheduletemp in Context.ProjectScheduleTemplate.Where(t => t.DeletedBy == null) on
                    projectschedule.Id equals projectscheduletemp.ProjectScheduleId
                join period in Context.ProjectDesignPeriod on projectscheduletemp.ProjectDesignPeriodId equals period.Id
                    into gj
                from subpet in gj.DefaultIfEmpty()
                join visit in Context.ProjectDesignVisit on projectscheduletemp.ProjectDesignVisitId equals visit.Id
                join template in Context.ProjectDesignTemplate on projectscheduletemp.ProjectDesignTemplateId equals
                    template.Id
                join variable in Context.ProjectDesignVariable on projectscheduletemp.ProjectDesignVariableId equals
                    variable.Id
        ...
        {
            //int id = 0;
            var referenceVariable = (from projectScheduletemp in Context.ProjectScheduleTemplate.Where(x =>
                    x.ProjectDesignVariableId == projectDesignVariableId && x.DeletedBy == null)
                join projectSchedule in Context.ProjectSchedule.Where(x => x.DeletedBy == null) on projectScheduletemp
